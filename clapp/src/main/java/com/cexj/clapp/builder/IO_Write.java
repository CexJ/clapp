...
public final class IO_Write<T,F extends FunctionFromFuture<T,?>,G extends FunctionFromFuture<?,?>,R, N> {

	private final IO<T,F,G,R,N> io;
	
	private IO_Write(final IO<T,F,G,R,N> io) {
		this.io = io;
	}
	
	static <T,F extends FunctionFromFuture<T,?>,G extends FunctionFromFuture<?,?>,R,N> IO_Write<T,F,G,R,N> of(final IO<T,F,G,R,N> io) {
	...
	public IO_Parallel<Future<T>, FunctionFromFuture<Future<T>, N>, G, R, N> inParallel() {
